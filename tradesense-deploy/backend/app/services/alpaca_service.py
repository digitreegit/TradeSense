...
class AlpacaService:
    ...
    def get_account(self) -> dict:
        ...
        try:
            ...
            raw_pv     = float(account.portfolio_value or account.equity)
            initial    = settings.initial_capital  # 10,000

            # Alpaca paper starts at $100K; we treat our $10K as 100% base.
            position_value  = raw_pv - raw_cash
            managed_equity  = initial + position_value
            profit_loss     = managed_equity - initial
            profit_loss_pct = (profit_loss / initial) * 100

            return {
                "equity":          round(managed_equity, 2),
                "cash":            round(min(raw_cash, initial), 2),
                "buying_power":    round(min(float(account.buying_power), initial * 2), 2),
                "portfolio_value": round(managed_equity, 2),
    ...
    def get_bars(self, symbol: str, timeframe: str = "1Day", limit: int = 100) -> list:
        ...
        # Try IEX first (free tier), then SIP (paid)
    ...
    def _get_current_price(self, symbol: str) -> float:
        """Get current price from snapshot, fallback to hardcoded."""
        try:
            from alpaca.data.requests import StockSnapshotRequest as SSR
            req = SSR(symbol_or_symbols=symbol)
