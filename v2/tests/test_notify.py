...
def test_send_test_returns_telegram_error_description():
    me = MagicMock()
    me.json.return_value = {"ok": True, "result": {"username": "TradeSenseDigtreeBot"}}
    fail = MagicMock()
    fail.text = "bad"
    ...
    with patch.object(notify.settings, "telegram_bot_token", "8530546766:AAHga2OQ-Okf2SfLCHDtAp7GEEJ1ZXLhafY"), \
         ...
         patch("app.notify.httpx.post", side_effect=[me, fail]):
...
def test_send_strips_quoted_chat_id():
    resp = MagicMock()
    resp.json.return_value = {"ok": True}
    with patch.object(notify.settings, "telegram_bot_token", " tok "), \
         patch.object(notify.settings, "telegram_chat_id", '"8788110870"'), \
         patch("app.notify.httpx.post", return_value=resp) as post:
        assert notify.send("hello") is True
        assert post.call_args.kwargs["json"]["chat_id"] == 8788110870
