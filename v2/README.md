...
대시보드 우상단 **CRYPTO** 버튼은 분석·시그널만 제공합니다(주문 없음 —
Alpaca가 이 지역에서 크립토 매매를 지원하지 않아 실행은 로빈후드에서 수동).
BTC 추세 + 시장 폭으로 강세/혼조/약세를 판정하고, 변동성 상위·상승 추세
코인에 RuleFive식 레벨(1차 매수 / −step 추가 매수 / +step 익절 / 50EMA
이탈 청산)을 제시합니다. step은 코인별 실현 변동폭에서 산출(4~12%),
기준 예산 $1,000, 약세장에서는 절반만 배분합니다.
