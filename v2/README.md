...
**쌀 때 사고, 비쌀 때 판다.** — 주식은 Alpaca 자동매매, 크립토는 로빈후드 수동 실행 + AI 가이드.
...
### 크립토 어드바이저 (로빈후드 · 수동 매매)

Alpaca가 이 지역에서 크립토 매매를 지원하지 않으므로, v3에서는 **크립토 탭**이
기본 화면입니다. 로빈후드 계좌·보유 화면 **스크린샷**을 올리면 AI가 현금·수량·
평단을 읽고 장부를 갱신한 뒤 매수/매도 가이드를 보여줍니다. 매일 09:00·12:00·
21:00 (ET) 자동 점검 + 텔레그램 알림. 로빈후드에서 실행한 뒤 대시보드 **확인**
(실제 체결 금액 입력)을 눌러야 장부에 반영됩니다.
