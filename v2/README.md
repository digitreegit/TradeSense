...
Alpaca가 이 지역에서 크립토 매매를 지원하지 않으므로, 봇이 포트폴리오를
분석해 "뭘 얼마치 산다/판다"를 지시하고 실행만 사용자가 로빈후드에서
합니다. 매일 09:00·12:00·21:00 (ET) 자동 점검하며, 세 번 모두 텔레그램으로
알립니다. 장부는 추천을 바로 반영하지 않고, 대시보드에서 실행 **확인**
(실제 체결 금액 입력 가능)을 눌러야 맞춰집니다. **CRYPTO** 버튼의
"실보유 등록"에 실제 로빈후드 수량·평단·현금을 입력하면 그 실계좌 기준으로
장부가 재구성되고, 주문 크기도 실제 계좌 규모에 맞춰 산출됩니다.

로직: 일차 목표는 원금 회복. 상승 추세(50EMA 위 + 20EMA≥50EMA) 코인 중
30일 수익률(상대강도) 상위 최대 4개, 코인별 step(4~12%)으로 RuleFive식
운영 — +step 익절, −step 1회 추가 매수(단, 평단 대비 15% 이상 손실이면
물타기 금지), 50EMA 이탈 시 전량 정리. 약세장에는 진입 크기를 75%로 줄이고
현금 15%는 남겨 둡니다. 한 코인이 계좌의 35%를 넘으면 점검마다 최대 45%까지
단계적 축소를 지시하고, 확보된 현금은 상승 상대강도 코인으로 바로 재배치합니다.
$50 미만 소액은 상승 추세가 아니면 정리합니다.
